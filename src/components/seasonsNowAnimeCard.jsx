...
const getData = async () => {
    const data = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}seasons/now?limit=8`);

    return data.json();
};


export default async function seasonsNowAnimeCard() {
    ...
    return (
        <>
            <div className="flex justify-between items-center px-12 mt-6">
                ...
                <Link href="/seasonNow" className="text-white bg-neutral-800 p-2 rounded-full hover:bg-neutral-700 hover:text-orange-500">more+</Link>
            </div>
